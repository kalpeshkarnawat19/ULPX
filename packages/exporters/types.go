...
// EventInfo represents core event metadata in ULPF Canonical IR.
...
// SourceInfo represents originating device/agent metadata.
...
// Endpoint represents a network host/port endpoint.
...
func (e *NormalizedEvent) Validate() error {
	if e == nil {
		return fmt.Errorf("normalized event cannot be nil")
	}
...
// DeepCopy creates an independent, deep copy of NormalizedEvent.
func (e *NormalizedEvent) DeepCopy() *NormalizedEvent {
	if e == nil {
		return nil
	}
...
// FromJSON deserializes JSON bytes into NormalizedEvent.
func FromJSON(data []byte) (*NormalizedEvent, error) {
	var ev NormalizedEvent
	if err := json.Unmarshal(data, &ev); err != nil {
		return nil, fmt.Errorf("failed to parse NormalizedEvent: %w", err)
	}
