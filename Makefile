...
.PHONY: test-contracts test-ingest test-firewall test-parser test-ir test-lineage test-exporters test-passport test-detection-contracts test-dps test-profiler test-mapper test-ai-assist test-compiler test-onboarding test-validation test-drift test-semantic-drift test-shadow test-ml test
...
test-semantic-drift:
	$(PYTHON) -m pytest ml/tests/test_semantic_drift.py -v

test-shadow:
	$(PYTHON) -m pytest ml/tests/test_shadow.py -v

test-ml:
	$(PYTHON) -m pytest ml/tests/ -v

test: test-contracts test-ingest test-firewall test-parser test-ir test-lineage test-exporters test-detection-contracts test-dps test-passport test-drift test-semantic-drift test-shadow test-ml
